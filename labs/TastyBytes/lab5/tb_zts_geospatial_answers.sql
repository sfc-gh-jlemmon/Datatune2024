/***************************************************************************************************
Adapted from: https://quickstarts.snowflake.com/guide/tasty_bytes_zero_to_snowflake_geospatial/index.html
...
-- Let's find the types of categories that Safegraph tracks
SELECT * FROM frostbyte_safegraph.public.frostbyte_tb_safegraph_s cpg LIMIT 10;
...
-- TASK:  Find a count of places by category in a city (Paris, London, )
-- Hint: Here are the cities in this data set:
SELECT DISTINCT cpg.city FROM frostbyte_safegraph.public.frostbyte_tb_safegraph_s cpg;

-- YOUR WORK GOES HERE --
SELECT cpg.top_category, cpg.sub_category, count(*) 
FROM frostbyte_safegraph.public.frostbyte_tb_safegraph_s cpg
WHERE cpg.city = 'London'
GROUP BY ALL;
...
-- For Tasty Bytes, let's find significant tourist attractions in Paris
SELECT 
    cpg.placekey,
    cpg.location_name,
    cpg.longitude,
...
-- Test our new view
SELECT * FROM frostbyte_tasty_bytes.analytics.orders_v 
WHERE city = 'Paris' 
LIMIT 10;
...
-- NOTE:  You have to use the geometric_center_point from the previous command!
SET center_point = '';
...
-- TASK: Create a view called FROSTBYTE_TASTY_BYTES.ANALYTICS.TOP_10_TRUCKS that contains 
--      the following for the top 25 locations (by sales) in a city of your choosing:
-- 
--      Truck Brand Name
--      Geo Point (geography)
--      Latitude
--      Longitude
--      sum of that location's sales



-- YOUR WORK GOES HERE --
CREATE OR REPLACE VIEW FROSTBYTE_TASTY_BYTES.ANALYTICS.TOP_10_TRUCKS AS 
WITH _top_10_locations AS 
(
    SELECT TOP 25
        o.location_id,
    ...
    FROM frostbyte_tasty_bytes.analytics.orders_v o
    WHERE 1=1
        AND o.primary_city = 'London'
        AND YEAR(o.date) = 2022
    GROUP BY o.location_id, o.latitude, o.longitude, o.truck_brand_name
    ORDER BY total_sales_usd DESC
) 
SELECT * FROM _top_10_locations;

SELECT * FROM FROSTBYTE_TASTY_BYTES.ANALYTICS.TOP_10_TRUCKS;
